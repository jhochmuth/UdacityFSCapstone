...
def test_delete_actor(client):
    ...
    res = client.delete('/actors/{}'.format(actor.id),
                        headers=EXECUTIVE_PRODUCER_HEADER)
...
def test_delete_actor_nonexistent(client):
    res = client.delete('/actors/{}'.format(99999),
                        headers=EXECUTIVE_PRODUCER_HEADER)
...
def test_delete_movie(client):
    ...
    res = client.delete('/movies/{}'.format(movie.id),
                        headers=EXECUTIVE_PRODUCER_HEADER)
...
def test_delete_movie_nonexistent(client):
    res = client.delete('/movies/{}'.format(99999),
                        headers=EXECUTIVE_PRODUCER_HEADER)
...
def test_patch_actor(client):
    ...
    res = client.patch('/actors/{}'.format(actor_id),
                       headers=EXECUTIVE_PRODUCER_HEADER,
                       json=updated_actor)
    ...
    assert actor.name == 'actor' \
        and actor.age == 26 and actor.gender == 'female'


def test_patch_actor_nonexistent(client):
    updated_actor = {'name': 'actor', 'age': 26, 'gender': 'female'}
    res = client.patch('/actors/{}'.format(99999),
                       headers=EXECUTIVE_PRODUCER_HEADER,
                       json=updated_actor)
...
def test_patch_movie(client):
    ...
    res = client.patch('/movies/{}'.format(movie_id),
                       headers=EXECUTIVE_PRODUCER_HEADER,
                       json=updated_movie)
...
def test_patch_movie_nonexistent(client):
    release = datetime.now()
    updated_movie = {'title': 'updated movie name', 'release': release}
    res = client.patch('/movies/{}'.format(99999),
                       headers=EXECUTIVE_PRODUCER_HEADER,
                       json=updated_movie)
...
def test_delete_actor_assistant(client):
    res = client.delete('/actors/{}'.format(1),
...
def test_delete_movie_director(client):
    res = client.delete('/movies/{}'.format(1),
                        headers=CASTING_DIRECTOR_HEADER)
